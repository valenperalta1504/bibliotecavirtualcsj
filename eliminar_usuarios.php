...
onload="mostrarVentana()"> <!-- Invocamos la función mostrarVentana() al cargar la página -->
    ...
    function confirmarAccion() {
        ...
        $.ajax({
            url: "eliminar_reservaciones.php",
            type: "POST",
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["accion"]) && $_POST["accion"] === "confirmar") {
    ...
    session_start();
$id = $_SESSION['eliminar_reserva'];

unset($_SESSION['eliminar_reserva']);

$query = "SELECT ISBN FROM reservaciones WHERE id = $id";
$result = mysqli_query($conn, $query);
if ($result) {
